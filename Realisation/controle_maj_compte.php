...
<html>
    ...
    <body>
        ...
        //LES ERREURS DE COMPTABILITé [a-zA-Z\s]+.[a-zA-Z0-9]
        $errors1=[];
        //Login
        ...
        if(!isset($err["Nom"])){if(!preg_match("#^[a-zA-Z\s]+$#",$_POST["maj_Nom"])){$errors1["Nom"]='Le champ *Nom* doit contenir que des LETTRES .';}}
        if(!isset($err["Prenom"])){if(!preg_match("#^[a-zA-Z\s]+$#",$_POST["maj_Prenom"])){$errors1["Prenom"]='Le champ *Prenom* doit contenir que des LETTRES .';}}

        
       //L'ERREUR DE LOGIN DEJA EXISTANT DANS LA BASE DE DONNEES
        $existe=is_existe_logine($_POST["maj_Login"],$lien);
            if(count($err)!=4){ 
